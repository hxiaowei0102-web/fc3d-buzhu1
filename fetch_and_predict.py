...
def fetch_apihz():
    ...
    return [(item["code"], *[int(x) for x in item.get("result","").split()[:3]]) for item in items if len(item.get("result","").split()) >= 3]
...
def fetch_with_fallback():
    sources = {"huiniao.top": fetch_huiniao, "cwl.gov.cn": lambda: fetch_cwl(10),
               "apihz.cn": fetch_apihz, "55128.cn": fetch_55128}
    results_by_source = {}
...
if __name__ == "__main__":
    print(f"=== FC3D Cloud v4 === {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC")
    print(f"requests: {'OK' if HAS_REQUESTS else 'NO'}\n")
